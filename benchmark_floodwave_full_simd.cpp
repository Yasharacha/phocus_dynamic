...
inline double flux(double u) { double up = std::max(u, 0.0); return up * std::sqrt(up); }
...
double compute_cfl_serial(const std::vector<double>& u, double dt, double dx) {
    double umax = 0.0;
    for (double ui : u) umax = std::max(umax, std::abs((ui <= 0.0) ? 0.0 : 1.5 * std::sqrt(ui)));
    return umax * dt / dx;
}
...
double compute_cfl_simd(const std::vector<double>& u, double dt, double dx) {
    ...
    for (int i = 0; i < N; ++i) {
        double ws = (u[i] <= 0.0) ? 0.0 : 1.5 * std::sqrt(u[i]);
        umax = std::max(umax, std::abs(ws));
    }
...
int main() {
    ...
    std::ofstream csv("benchmark_floodwave_full_simd.csv");
    if (!csv) {
    ...
    std::cout << "\nWrote: benchmark_floodwave_full_simd.csv\n";
    return 0;
}
