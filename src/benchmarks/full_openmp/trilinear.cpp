...
inline double flux(double u) { return -std::max(-u, 0.0) + std::max(u - 1.0, 0.0); }
...
double compute_cfl_serial(const std::vector<double>& u, double dt, double dx) {
    double umax = 0.0;
    for (double ui : u) umax = std::max(umax, 1.0);
    return umax * dt / dx;
}
...
int main() {
    ...
    std::ofstream csv("benchmark_trilinear_full_openmp.csv");
    if (!csv) {
    ...
    std::cout << "\nWrote: benchmark_trilinear_full_openmp.csv\n";
    return 0;
}
