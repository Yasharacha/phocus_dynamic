...
double compute_cfl_serial(const std::vector<double>& u, double dt, double dx) {
    double umax = 0.0;
    for (double ui : u) umax = std::max(umax, std::abs(flux_prime(ui)));
    return umax * dt / dx;
}
...
double compute_cfl_omp(const std::vector<double>& u, double dt, double dx) {
    ...
    for (int i = 0; i < N; ++i) {
        umax = std::max(umax, std::abs(flux_prime(u[i])));
    }
    return umax * dt / dx;
}
...
int main() {
    ...
    std::ofstream csv("benchmark_buckley_full_openmp.csv");
    if (!csv) {
    ...
    std::cout << "\nWrote: benchmark_buckley_full_openmp.csv\n";
    return 0;
}
