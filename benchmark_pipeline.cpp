...
double compute_cfl(const std::vector<double>& u, double dt, double dx) {
    double umax = 0.0;
    for (double val : u) umax = std::max(umax, std::abs(val));
    return umax * dt / dx;
}
...
// -----------------------------------------------------------------------
// Serial: step + CFL + TV all sequential each timestep
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Pipeline: producer runs the given step kernel; consumer runs CFL+TV.
//
// While producer computes step n+1, consumer computes CFL+TV for step n.
// The step kernel can be either step_serial or step_omp (caller's choice).
//
//   step 1:  [kernel──────────]
//   step 2:                   [kernel──────────]
//   cfl/tv 1:       [─cfl+tv─]
//   cfl/tv 2:                        [─cfl+tv─]
//
// Timing covers only the producer loop so it reflects the wall-clock
// bottleneck (producer blocks if consumer hasn't drained the slot).
// -----------------------------------------------------------------------
...
double run_pipeline(StepFn step_fn, int N, int n_steps, double dt, double dx) {
    ...
    // ---- Consumer: waits for filled buffer, computes CFL + TV ----
    std::thread consumer([&]() {
        for (int n = 0; n < n_steps; ++n) {
            {
    ...
    // ---- Producer: runs step kernel, hands result to consumer ----
    std::vector<double> u(N), u_new(N);
    for (int i = 0; i < N; ++i) u[i] = initial_condition(x[i]);
...
int main() {
    ...
    // column widths
    const int CW = 14;  // time column
    const int SW = 10;  // speedup column
    ...
    for (int n_steps : steps_list) {
        ...
        for (int N : ns) {
            ...
            // --- pipeline + OMP step at each thread count ---
